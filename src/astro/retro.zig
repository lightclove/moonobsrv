...
pub fn upcoming(body: planets.Body, from_unix: i64, out: *[max_windows]?Window) usize {
    ...
    if (in_retro) {
        // ищем фактическую станцию назад (ретро длится < 25 суток)
        var found = false;
        var tb = jd0;
        while (jd0 - tb < 25.0) : (tb -= 0.5) {
            if (speed.eval(tb) >= 0) {
...
fn makeWindow(body: planets.Body, start_jd: f64, end_jd: f64) Window {
    ...
    // Пред-тень: идём назад до градуса станции директа (лам_енд).
    const pre_fn = LonFn{ .body = body, .target = lam_end };
    var pre_jd = start_jd;
    var tp = start_jd - 1.0;
    while (start_jd - tp < 30.0) : (tp -= 1.0) {
        if (pre_fn.eval(tp) < 0) {
