...
pub const slides = [_]Slide{
    ...
    .{
        ...
        \\1-й день начинается в новолуние, N-й — когда элонгация Луны
        \\(угол от Солнца) проходит отметку (N−1)·12°.
        \\
        \\Каждый лунный день длится от ~21 до ~26 часов —
        \\границы не совпадают с календарными сутками.
        \\
        \\Некоторые календари считают дни от восхода Луны в конкретном городе —
        \\тогда границы чуть отличаются, номер дня тот же. Бот считает
        \\астрономически, от новолуния.
        ,
    },
    .{
        .title = "📡 Команда /day",
        ...
        \\Синонимы: <code>/moon</code>, <code>/луна</code>
        \\
        \\В ответе:
        \\• какой идёт лунный день (из 30)
        \\• когда начался и когда закончится
        \\• знак Луны и фаза (молодая, полная, старая…)
        \\• % освещённости
        \\
        \\С датой: <code>/day 1.01.2027</code>
        \\Подписчики получают уведомление в момент начала каждого дня.
        ,
    },
    .{
        ...
        \\• станциях Меркурия (разворот назад/вперёд)
        \\• начале лунного дня
        \\
    ...
    .{
        ...
        \\
        \\Долготы всех классических планет на сейчас: Меркурий…Сатурн,
    ...
    .{
        ...
        \\
        \\Пока ждёте решения: «Заявка отправлена. Ждите решения.»
    ...
    .{
        ...
        \\→ нет, от ~21 до ~26 часов, границы каждый день другие.
        \\
    ...
    .{
        ...
        \\• <code>/day</code> — лунный день (/moon, /луна)
...
pub const admin_slides = [_]Slide{
    .{
        ...
        \\
        \\В ответе — сумма простоя и промежутки «с … по …» по каждому слою.
...
pub fn render(w: *std.Io.Writer, i: usize, admin: bool) !void {
    const total = n(admin);
    const s = slideAt(i, admin);
    try w.print("{s}\n", .{s.title});
    try writeBar(w, i, total);
    try w.print("  <b>{d}/{d}</b>\n──────────────────\n\n{s}", .{ i + 1, total, s.body });
}
